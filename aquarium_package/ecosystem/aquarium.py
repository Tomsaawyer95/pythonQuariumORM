...
from aquarium_package.factories import fishs_factory, create_fish_type
...
class Aquarium:
    ...
    def add_fish(self, fish: FishORM | None = None):
        ...
        """
        if fish is None:
            fish = fishs_factory.create_fish_type(fish_aquarium_id=self.id)
        elif fish.type_fish is None:
            fish = fishs_factory.create_fish_type(
                fish_aquarium_id=self.id,
                age=fish.age,
                name=fish.name,
                pv=fish.pv,
                sexe=fish.sexe,
    ...
    def add_algue(self, age=0, pv=5):
        ...
        Returns:
        """
        algue = AlgueORM(algue_aquarium_id=self.id, pv=pv, age=age)
    ...
    def handle_algue_duplication(self, algue: AlgueORM) -> None:

        if algue.pv < 10:
            return

        new_algue = algue.duplicate()
        self.management_session.add_to_session(new_algue)
        self.management_session.flush_to_session(new_algue)
        new_algue.name = f"Algue-{new_algue.id}"
        self.algues_list.append(new_algue)
